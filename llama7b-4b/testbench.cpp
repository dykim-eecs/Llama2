...
int main() {
    ...
    build_transformer(transformer, "llama7b-4b.bin");
...
void read_checkpoint(std::string checkpoint, Config *config,
    ...
    if (version != 3) { fprintf(stderr, "Unsupported version %d\n", version); exit(1); }
    int header_size = 256;
    ...
    auto read_qtensor = [&](auto &tensor, int count, int size_each) {
        // 4비트 양자화 데이터는 8비트 데이터의 절반 크기입니다.
        int quantized_size = size_each / 2;
        // 스케일링 팩터의 크기는 그룹 크기에 따라 결정됩니다.
        constexpr int GS_const = 64;
        int scale_size = size_each / GS_const;

         for (int i = 0; i < count; i++) {
            // 양자화된 값을 읽을 때 크기를 절반으로 변경
            fread(tensor[i].q, sizeof(int8_t), quantized_size, file);
            // 스케일링 팩터를 읽을 때 크기를 변경
            fread(tensor[i].s, sizeof(float), scale_size, file);
        }
    };
